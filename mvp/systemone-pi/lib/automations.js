...
function validateAutomation(input, registry) {
  ...
  if (input.trigger?.type !== 'device') throw automationError('Nur lokale Geräteereignisse werden in v1 unterstützt.');
  const trigger = { type: 'device', ...validatePredicate(input.trigger, 'Trigger') };
  ...
  for (const id of [trigger.deviceId, ...conditions.map(x => x.deviceId), ...actions.map(x => x.deviceId)]) if (!registry.get(id)) throw automationError(`Gerät ${id} wurde nicht gefunden.`, { deviceId: id });
...
function fromTemplate(id, input, registry) {
  ...
  const triggerDevice = registry.get(input.triggerDeviceId), actionDevice = registry.get(input.actionDeviceId);
  if (!triggerDevice || !template.triggerProfiles.includes(triggerDevice.profile) || !actionDevice || !template.actionProfiles.includes(actionDevice.profile)) throw automationError('Geräte passen nicht zur gewählten Vorlage.');
...
class AutomationEngine extends EventEmitter {
  ...
  async handleDeviceChange(deviceId) {
    ...
    for (const automation of this.automations) {
      if (!automation.enabled || automation.trigger.deviceId !== deviceId || this.running.has(automation.id) || !this.matches(automation.trigger) || !automation.conditions.every(value => this.matches(value))) continue;
