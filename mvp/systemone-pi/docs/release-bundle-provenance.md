...
Beim Prozessstart wird ein vorhandenes `RELEASE-MANIFEST.json` nochmals vollständig geprüft. Hash-, Modus-, Paketnamen- oder Versionsabweichungen blockieren den Start mit `BUILD_IDENTITY_INVALID` beziehungsweise `BUILD_IDENTITY_MISMATCH` und Exitcode 78. Health-API, lokale Diagnose und App zeigen anschließend Version, Release-/Entwicklungskanal, Verifikationsstatus und den gekürzten Quell-Commit. Ein Entwicklerlauf ohne Manifest bleibt ausdrücklich als unverifizierter `development`-Build erkennbar und erfindet keine Commitidentität.
