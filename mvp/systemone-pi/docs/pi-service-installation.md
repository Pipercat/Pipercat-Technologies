...
Der Start ist **fail-closed**: ungültiger Port, unbekannte Modi, andere Boolean-Werte als exakt `true`/`false`, nur ein gesetzter TLS-Pfad, fehlende TLS-/Update-Schlüsseldateien, öffentliche Hue-IP oder realer Govee-Modus stoppen den Prozess mit verständlichem Fehler. Insbesondere fällt eine unvollständige TLS-Konfiguration niemals still auf HTTP zurück. `ExecStartPre` führt denselben Check vor der Initialisierung aus. Konfigurationsfehler enden mit standardisiertem `EX_CONFIG`-Exitcode 78; Port-/Adresskonflikte mit 69 und fehlende Bindrechte mit 77. `RestartPreventExitStatus=69 77 78` verhindert für diese dauerhaft zu korrigierenden Startfehler eine systemd-Neustartschleife. Nach der Korrektur wird der Dienst bewusst mit `systemctl restart systemone-pi` erneut gestartet.
