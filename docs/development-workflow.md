...
## 3b. Betriebshinweis: Spotlight-/iCloud-Indizierung kann Python-Imports extrem verlangsamen

In einer KI-Sandbox auf macOS mit `~/Documents` unter iCloud Drive wurde beobachtet, dass ein simples `import fastapi` (bzw. `import app.audit` u. Ä.) **60–140 Sekunden** dauern kann, obwohl dieselbe Suite Minuten zuvor in unter 4 Sekunden durchlief — Ursache war `mds_stores` (Spotlight-Indizierung), das nach dem Anlegen eines neuen `.venv`/`.venv312` (tausende neuer Dateien) kurzzeitig auf >95 % CPU lief, kombiniert mit aktiver iCloud-Sync-Aktivität (`bird`, `cloudd`). Es handelt sich um echtes Warten (I/O-Blockade), nicht um einen Deadlock — `time` zeigt dabei nahe-0 CPU-Zeit bei hoher Wall-Clock-Zeit. **Abhilfe:** eine leere Datei `.metadata_never_index` im jeweiligen `.venv`-Verzeichnis anlegen (von Spotlight respektierte Konvention), danach kurz abwarten, bis `mds_stores` seine aktuelle Warteschlange abgearbeitet hat. Kein Repository-Fehler; bei Bedarf für spätere KIs: vor dem Debuggen eines scheinbar hängenden Testlaufs zuerst `ps aux | grep mds_stores` prüfen, bevor Code-Ursachen vermutet werden.
