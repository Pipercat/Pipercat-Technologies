...
|---|---|---|
| `apps/customer-backend` | `pytest` (mit `DATABASE_URL` gesetzt, Python 3.12 empfohlen — siehe `docs/architecture/data-model.md` für einen Python-3.14-Sandbox-Hinweis; MQTT-Tests brauchen zusätzlich ein lokales `mosquitto`) | 59 passed (56 aus `S1V2-01-003`–`S1V2-02-003` + 3 neue MQTT-Integrationstests aus `S1V2-02-004`, siehe `docs/architecture/mqtt-eventbus.md`) |
